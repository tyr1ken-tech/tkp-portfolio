...
const Navbar = () => {
    ...
    return (
        <div
            className="fixed top-6 left-1/2 transform -translate-x-1/2 z-50 w-[98%]"
            style={{ maxWidth: '88rem' }}
        >
            <nav className="flex items-center justify-between w-full px-6 py-3 md:px-8 md:py-4 bg-[#181C21] border border-[#a4a4a4]/25 rounded-[11px] backdrop-blur-md transition-all duration-300">

                {/* Left: Logo */}
                <div className="flex-shrink-0 cursor-pointer">
                    <span className="font-poppins font-extrabold text-[24px] text-white">
                        TKP.
                    </span>
                </div>
                    ...
                    {navLinks.map((item) => (
                        <li key={item}>
                            <button
                                ...
                                className={`font-poppins font-medium text-[20px] tracking-[-0.02em] transition-all duration-300 px-4 py-2 ${active === item
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    <button
                        className="flex items-center justify-center text-white hover:text-[#00FFE1] transition-colors p-2 rounded-full hover:bg-white/5"
